...
# Add staged module to path
_STAGED_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
)
sys.path.insert(0, _STAGED_DIR)

# Also add the parent reports/staged if running from tests/kernel/
_ALT_STAGED = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    "..", "..", "reports", "staged"
)
if os.path.isdir(_ALT_STAGED):
    sys.path.insert(0, os.path.abspath(_ALT_STAGED))
...
def warmed_brain_db(brain_db):
    ...
    now = time.time()
    # Insert 5001 rows to pass warmup gate
...
def test_error_path_no_crash(tmp_path, cieu_db):
    ...
    assert "BRAIN_WRITEBACK_FAILED" in str(result.get("cieu_event_ids", []))  \
        or result["error"] is not None  # At minimum, error field is set
