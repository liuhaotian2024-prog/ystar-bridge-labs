...
### 14.4-bis Sequence + State Machine (Board 2026-04-22 第 10 catch — 真 sequence 不是 phase 切片)

老大 10 catch: phase × dim 矩阵 ≠ 真 sequence. 真 architecture 是 3-stage sequence + Phase B 是 3-state in-flight loop:

```
A. 方向定 (Phase 1 pre-flight)
   agent 写 navigation (Y*_t + m_functor + 计划)
   → y_star_self_check.py 自验 (Leo W12-3)
   → PreToolUse hook pre-flight 治理审 (Wave-3 Maya 集成)
   → PASS 进 B / FAIL recipe + rewrite loop A
   ↓

B. 实时追 (Phase 2 in-flight, 3-state continuous loop)
   per tool call:
   ├─ STATE 1 做对了默认: forget_guard PASS + observable_action 有 evidence → 继续
   │
   ├─ STATE 2 做错了: forget_guard 实时 deny / narrative_coherence 抓 claim≠tool / claim_mismatch 抓 metadata mismatch
   │  → recipe (现 LIVE 37/37) + redirect (现 LIVE intervention_engine / router_registry / omission redirect_id)
   │  → cieu_decision_normalizer + exp3_b3_rewrite_fire 触发 rewrite
   │  → loop back to A (重审 navigation) 或 in-place 修 next tool call
   │
   └─ STATE 3 漏做了 (in-flight): omission engine 8 RULE_* 实时检 tracked entity overdue
      → P0 alarm + force complete (现 LIVE)
      → 不等 Phase C, 实时 in-flight 强制
   ↓

C. 真做完 confirm (Phase 3 complete)
   final verify: K9 silent fire audit + unified_compliance_audit + amendment_coverage_audit + counterfactual_engine
   → 真做完 + 无 silent error = atomic close 进 next
   → 否则 Phase B loop 再来 (omission 强制 / commission rewrite)
```

**核心校正**:
- Phase B 是 **3-state 实时 active enforce**, 不是 "实时 deny + 事后 audit" 分裂
- omission 实时 force 不等 Phase C (避 "忙到结束才发现漏"), 这是与 omission_engine 现 LIVE 行为 align — RULE_DELEGATION/ACKNOWLEDGEMENT 等 8 RULE_* 都是 deadline-based 实时 watch
- Phase C 仅 "final confirm + audit" — 大部分 commission/omission 已在 B 实时 loop close 完, C 只 catch 最后没 catch 的 silent failure
- "做错" + "漏做" 都是 in-flight 实时 enforce (B 同步), 不是 phase 切片

### 14.4-ter sequence vs 切片矩阵的工程意义

我之前 6-cell 矩阵 → 工程上易把"做错"全推给 post-execute audit, 把"漏做"全推给 post-completion check. 这是**事后兜底姿态**, 不是 in-flight active.

老大 sequence 视角 → Phase B 是**实时 3-state machine**, agent 每 tool call 都过同步 (做对 / 做错 / 漏做) 三态 check. 这才是真 active enforce. 现有 components 行为本来就这样跑 (forget_guard PreToolUse 实时 + omission deadline 实时), **只是我 spec 表述切片化错位了**, 误导工程方向. 修订 spec 还原 sequence active 姿态.
