#!/usr/bin/env python3
"""
Audience: Board (executes before Ctrl-C/exit). CEO authored; Board-shell invokes per CLAUDE.md session-close protocol.
Research basis: CLAUDE.md session-close mandates 3 steps (delete session markers + run session_close_yml.py + run twin_evolution per Board participation). Today's session 10+ hours, Board participated heavily (~50+ prompts), restart authorized by Board 2026-04-19 late-evening.
Synthesis: run all 3 close steps + update memory/session_handoff.md with a honest state snapshot (what's live, what's partial, what's next). scripts/ + memory/ are cross-boundary for CEO scope, so Board-shell invokes.
Purpose: ensure fresh Aiden session boots with complete picture — NOT a clock-out summary but a handoff of in-flight + newly-live governance pieces.

Board Authorization: 2026-04-19 late-evening "退出再进来算新session" explicit stop predicate satisfied.
"""
...
# Step 1: delete session markers (per CLAUDE.md)
for f in ('scripts/.session_booted', 'scripts/.session_call_count'):
    p = COMPANY / f
    if p.exists():
        p.unlink()
        print(f'[1] removed {f}')
    else:
        print(f'[1] {f} absent (ok)')

# Step 2: update memory/session_handoff.md with today's honest state
handoff = COMPANY / 'memory' / 'session_handoff.md'
new_handoff = f'''# Session Handoff — 2026-04-19 → next Aiden session
...
(COMPANY / 'memory').mkdir(exist_ok=True)
handoff.write_text(new_handoff)
print(f'[2] updated {handoff} ({len(new_handoff)} chars)')

# Step 3: run session_close_yml.py ceo
print('[3] running session_close_yml.py ceo...')
out = subprocess.run(
    ['python3', str(COMPANY / 'scripts' / 'session_close_yml.py'), 'ceo',
     'Largest-ship day: 8 Ethan rulings + 20+ engineer ships + AMENDMENT-020 LIVE + 7 new constitutional rules + L1 pre-query wired. Board catches: binary thinking + clock-out disguise + whitelist/blacklist + priority drops. Ready for fresh session restart.'],
    capture_output=True, text=True, timeout=60
)
print(f'    exit={out.returncode}')
if out.stdout:
    print(f'    stdout (first 200): {out.stdout[:200]}')
if out.stderr and out.returncode != 0:
    print(f'    stderr (first 200): {out.stderr[:200]}')

# Step 4: run twin_evolution if script exists (Board participated heavily today)
twin = COMPANY / 'scripts' / 'twin_evolution.py'
if twin.exists():
    print('[4] running twin_evolution --mode extract-values...')
    out = subprocess.run(
        ['python3', str(twin), '--mode', 'extract-values'],
    ...
    print('[4] twin_evolution.py absent — skipping')

print('\n=== Session 2026-04-19 close complete. Board may exit + restart now. ===')
