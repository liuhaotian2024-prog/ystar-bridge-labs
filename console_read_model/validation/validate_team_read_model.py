...
def main() -> int:
    ...
    l6_12_unified_controlled_observation = generated_json.get(
        "console_read_model/generated/l6_12_unified_controlled_external_observation_evidence_loop_summary.json"
    )
    if l6_12_unified_controlled_observation:
        for field in [
            "l6_12_unified_controlled_external_observation_evidence_loop_complete",
            "mode",
            "run_classification",
            "selected_work_order_id",
            "backend_mode",
            ...
            "generated_capability_gap_registry",
            "generated_no_side_effect_receipt",
            "generated_readiness",
            "warning",
        ]:
            if field in l6_12_unified_controlled_observation:
                report.pass_(f"generated L6.12 unified observation field present: {field}")
            else:
                report.fail(f"generated L6.12 unified observation missing field: {field}")
        if (
            l6_12_unified_controlled_observation.get("mode")
            != "unified_controlled_external_observation_evidence_loop"
        ):
            report.fail("generated L6.12 summary must be unified controlled observation mode")
        ...
        if l6_12_unified_controlled_observation.get("run_classification") not in allowed_classifications:
            report.fail("generated L6.12 summary must use an allowed run classification")
        if (
            l6_12_unified_controlled_observation.get("selected_work_order_id")
            != "l6_10x_selected_work_order_001"
        ):
            report.fail("generated L6.12 summary must use the L6.10X selected work order")
        for field in [
            "l6_12_unified_controlled_external_observation_evidence_loop_complete",
            "fixture_proof_executed",
            "page_read_content_used_as_evidence",
            ...
            if l6_12_unified_controlled_observation.get(field) is not True:
                report.fail(f"generated L6.12 summary must keep {field}=true")
        for field in [
            "network_allowed",
            "real_observation_executed",
            "ask_user_for_url_occurred",
            ...
            if l6_12_unified_controlled_observation.get(field) is not False:
                report.fail(f"generated L6.12 summary must keep {field}=false")
        exact_values = {
        ...
        for field, expected_value in exact_values.items():
            if l6_12_unified_controlled_observation.get(field) != expected_value:
                report.fail(f"generated L6.12 summary must keep {field}={expected_value}")
        count_limits = {
            "query_count": l6_12_unified_controlled_observation.get("max_queries"),
            "search_results_considered": l6_12_unified_controlled_observation.get(
                "max_search_results_considered"
            ),
            "pages_opened": l6_12_unified_controlled_observation.get("max_pages_opened"),
            "domains_touched": l6_12_unified_controlled_observation.get("max_domains"),
            "crawl_depth_used": l6_12_unified_controlled_observation.get("max_crawl_depth"),
            "external_reads_used": l6_12_unified_controlled_observation.get(
                "max_total_external_reads"
            ),
            "evidence_packets_generated": l6_12_unified_controlled_observation.get(
                "max_evidence_packets"
            ),
        }
        for count_field, limit in count_limits.items():
            if limit is not None and l6_12_unified_controlled_observation.get(count_field, 0) > limit:
                report.fail(f"generated L6.12 summary exceeds runtime limit for {count_field}")
        for field in [
            "query_count",
            "search_results_considered",
            "pages_opened",
            ...
            if l6_12_unified_controlled_observation.get(field, 0) < 1:
                report.fail(f"generated L6.12 fixture summary must produce positive {field}")
        blockers = set(l6_12_unified_controlled_observation.get("blockers", []))
        for blocker in [
            "controlled_search_backend_not_configured",
            "controlled_public_page_read_adapter_not_configured",
        ]:
    ...
    l6_14_real_evidence_conflict_resolution = generated_json.get(
        "console_read_model/generated/l6_14_real_evidence_conflict_resolution_sprint_summary.json"
    )
    if l6_14_real_evidence_conflict_resolution:
        for field in [
            "l6_14_real_evidence_conflict_resolution_sprint_complete",
            "mode",
            ...
            "network_allowed",
            "safety_preflight_decision",
            "second_pass_real_observation_executed",
            "second_pass_queries_generated",
            "search_results_considered",
            "pages_opened",
            "domains_touched",
            "crawl_depth_used",
            "external_reads_used",
            "prior_real_evidence_packets",
            "new_real_evidence_packets",
            "total_real_evidence_packets",
            "conflicts_before",
            "conflicts_after",
            "unresolved_claims_before",
            ...
            "page_read_content_used_as_evidence",
            "secret_values_serialized",
            "generated_updated_mission_report",
            "generated_conflict_decision_packet",
            "generated_next_action_packet",
            "generated_no_side_effect_receipt",
            "generated_readiness",
            "warning",
        ]:
            if field in l6_14_real_evidence_conflict_resolution:
                report.pass_(f"generated L6.14 conflict sprint field present: {field}")
            else:
                report.fail(f"generated L6.14 conflict sprint missing field: {field}")
        if (
            l6_14_real_evidence_conflict_resolution.get("mode")
            != "real_evidence_conflict_resolution_sprint"
        ):
            report.fail("generated L6.14 summary must be conflict resolution sprint mode")
        allowed_l6_14_classifications = {
            "conflict_resolved",
            "conflict_bounded",
        ...
        if (
            l6_14_real_evidence_conflict_resolution.get("post_second_pass_classification")
            not in allowed_l6_14_classifications
        ):
            report.fail("generated L6.14 summary must use an allowed post-second-pass classification")
        if (
            l6_14_real_evidence_conflict_resolution.get("selected_work_order_id")
            != "l6_10x_selected_work_order_001"
        ):
            report.fail("generated L6.14 summary must use the L6.10X selected work order")
        for field in [
            "l6_14_real_evidence_conflict_resolution_sprint_complete",
            "artifact_refinement_candidate_generation_authorized",
        ]:
            if l6_14_real_evidence_conflict_resolution.get(field) is not True:
                report.fail(f"generated L6.14 summary must keep {field}=true")
        for field in [
            "ask_user_for_url_occurred",
            "external_side_effects_occurred",
            "core_writeback_occurred",
            ...
            if l6_14_real_evidence_conflict_resolution.get(field) is not False:
                report.fail(f"generated L6.14 summary must keep {field}=false")
        exact_l6_14 = {
            "max_selected_work_orders": 1,
        ...
        for field, expected_value in exact_l6_14.items():
            if l6_14_real_evidence_conflict_resolution.get(field) != expected_value:
                report.fail(f"generated L6.14 summary must keep {field}={expected_value}")
        count_limits = {
            "second_pass_queries_generated": l6_14_real_evidence_conflict_resolution.get("max_second_pass_queries"),
            "search_results_considered": l6_14_real_evidence_conflict_resolution.get(
                "max_search_results_considered"
            ),
            "pages_opened": l6_14_real_evidence_conflict_resolution.get("max_pages_opened"),
            "domains_touched": l6_14_real_evidence_conflict_resolution.get("max_domains"),
            "crawl_depth_used": l6_14_real_evidence_conflict_resolution.get("max_crawl_depth"),
            "external_reads_used": l6_14_real_evidence_conflict_resolution.get(
                "max_total_external_reads"
            ),
            "new_real_evidence_packets": l6_14_real_evidence_conflict_resolution.get(
                "max_new_real_evidence_packets"
            ),
        }
        for count_field, limit in count_limits.items():
            if limit is not None and l6_14_real_evidence_conflict_resolution.get(count_field, 0) > limit:
                report.fail(f"generated L6.14 summary exceeds runtime limit for {count_field}")
        if (
            l6_14_real_evidence_conflict_resolution.get("post_second_pass_classification")
            == "second_pass_blocked_by_backend_or_network"
            and not l6_14_real_evidence_conflict_resolution.get("blockers")
        ):
