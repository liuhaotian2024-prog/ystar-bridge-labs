...
def build_e22_ecosystem_alignment_gate(provider_sync: Dict[str, Any]) -> Dict[str, Any]:
    scans=[]
    ...
    return {"artifact_id":"e22_ecosystem_alignment_gate","closure_status":"ecosystem_aligned_no_cross_repo_changes_needed","repos_checked":scans,"gov_mcp_read_only":True,"gov_mcp_modified":False,"bridge_labs_modified_and_delivered":True,"Y_star_gov_immediate_mutation_needed":False,"ystar_company_future_migration_followup":True,"provider_capability_status":provider_sync["capability_status"],"drift_blockers":["live_provider_scaffolded_but_disabled","evidence_required_for_two_candidates"],"external_action_executed":False}

def render_ecosystem_alignment_gate(gate: Dict[str, Any]) -> str:
    lines=["# E22 Ecosystem Alignment Gate","",f"- closure_status: {gate['closure_status']}",f"- gov_mcp_read_only: {str(gate['gov_mcp_read_only']).lower()}",f"- gov_mcp_modified: {str(gate['gov_mcp_modified']).lower()}",f"- bridge_labs_modified_and_delivered: {str(gate['bridge_labs_modified_and_delivered']).lower()}","- external_action_executed: false","","## Repos"]
