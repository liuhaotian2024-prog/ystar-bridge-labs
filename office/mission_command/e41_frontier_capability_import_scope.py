...
def build_frontier_capability_import_scope() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_frontier_capability_import_scope")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_frontier_capability_import_scope()
    keys = [
...
def selected_capability_names(artifact: dict[str, Any] | None = None) -> list[str]:
    artifact = artifact or build_frontier_capability_import_scope()
    return [item.get("capability", "") for item in artifact.get("selected_imports", [])]
