...
def build_deep_research_method_enhancement() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_deep_research_method_enhancement")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_deep_research_method_enhancement()
    keys = [
...
def requires_lineage(claim: dict[str, Any]) -> bool:
    return not all(claim.get(key) for key in ["source_receipt_id", "cluster"])
