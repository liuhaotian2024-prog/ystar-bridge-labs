...
def build_whole_bridge_labs_runtime_archaeology() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_whole_bridge_labs_runtime_archaeology")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_whole_bridge_labs_runtime_archaeology()
    keys = [
...
def inspected_component_names(artifact: dict[str, Any] | None = None) -> list[str]:
    artifact = artifact or build_whole_bridge_labs_runtime_archaeology()
    return [item.get("component", item.get("path", "")) for item in artifact.get("canonical_runtime_components", [])]
