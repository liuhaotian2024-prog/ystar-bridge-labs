...
def build_cross_repo_alignment_after_frontier_import() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_cross_repo_alignment_after_frontier_import")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_cross_repo_alignment_after_frontier_import()
    keys = [
...
def alignment_proofs_pass(artifact: dict[str, Any] | None = None) -> bool:
    artifact = artifact or build_cross_repo_alignment_after_frontier_import()
    return all(bool(value) for value in artifact.get("proofs", {}).values())
