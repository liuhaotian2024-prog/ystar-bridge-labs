...
def build_canonical_ceo_runtime_map() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_canonical_ceo_runtime_map")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_canonical_ceo_runtime_map()
    keys = [
...
def component_statuses(artifact: dict[str, Any] | None = None) -> dict[str, str]:
    artifact = artifact or build_canonical_ceo_runtime_map()
    return {item["component"]: item.get("status", "unclear") for item in artifact.get("components", [])}
