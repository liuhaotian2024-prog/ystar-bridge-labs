...
def build_contradiction_topology_enhancement() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_contradiction_topology_enhancement")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_contradiction_topology_enhancement()
    keys = [
...
def is_forbidden_buyer_facing_claim(text: str, artifact: dict[str, Any] | None = None) -> bool:
    artifact = artifact or build_contradiction_topology_enhancement()
    lower = text.lower()
    return any(pattern.lower() in lower for pattern in artifact.get("buyer_facing_claims_forbidden", []))
