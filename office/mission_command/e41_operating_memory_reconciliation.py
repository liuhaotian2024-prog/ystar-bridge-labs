...
def build_operating_memory_reconciliation() -> dict[str, Any]:
    """Return the E41 artifact and keep a stable code-level entry point."""
    artifact = get_artifact("e41_operating_memory_reconciliation")
    return artifact


def forbidden_action_flags(artifact: dict[str, Any] | None = None) -> dict[str, bool]:
    artifact = artifact or build_operating_memory_reconciliation()
    keys = [
...
def write_surfaces(artifact: dict[str, Any] | None = None) -> list[str]:
    artifact = artifact or build_operating_memory_reconciliation()
    return [item["memory_surface"] for item in artifact.get("maps_recent_artifacts_to", []) if item.get("write_performed")]
