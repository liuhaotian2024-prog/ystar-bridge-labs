...
**`samantha_lin`** — 所有义务命令以此为 `--owner`。CIEU 审计中本岗位的义务记录全部以此 ID 为关键字。
...
```bash
python3.11 scripts/check_obligations.py --actor samantha_lin
python3.11 scripts/check_obligations.py --actor samantha_lin --overdue-only
```
