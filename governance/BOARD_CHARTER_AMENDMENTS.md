...
## AMENDMENT-004 — Single-Machine Operating Reality

| 字段 | 内容 |
|---|---|
| Level | 3（宪法伴随文档 · 架构变更影响 ≥2 个岗位核心职责） |
| 起草人 | Samantha Lin (Secretary) |
| 起草日期 | 2026-04-12 |
| Board 批准日期 | 2026-04-12 |
| Board 裁决 | 批准（三项签字位全部 approved） |
...
1. **点 1**：CLAUDE.md §156–167 "双机分工原则"→"单机运行原则"（§3.2 diff）. **APPROVED**.
2. **点 2**：CLAUDE.md §145–148 + §58–61 Windows 硬路径 → Mac OpenClaw 路径（§3.1 + §3.3 diff）. **APPROVED**.
3. **点 3**：授权 Secretary 按 §5.1 + §5.2 清单逐项同步，并追加本 AMENDMENT-004 条目. **APPROVED**.

### 执行结果

- CLAUDE.md 三处 diff 全部落地（§3.1 / §3.2 / §3.3）。
- `memory/session_handoff.md` "MAC mini" 标签替换为 "Single-Mac OpenClaw workspace"。
- §5.2 中优先清单按文件实况逐个审阅处理（详见本次 commit diff）。
- §5.3 明确不改的项目（`.ystar_session.json` Gemma endpoint、`scripts/local_learn.py` 的 192.168.1.228、`archive/deprecated/*`、`AGENTS.md`）保持原样。
- 历史报告（`reports/pretrain_verification_2026-04-05.md` 等）不回写，仅由本条目交叉引用声明其物理前提已失效。

### 交叉引用（历史前提已失效）

- `reports/pretrain_verification_2026-04-05.md`
- `reports/autonomous/2026-04-01-platform-mac-deployment.md`
- `reports/governance_layer_on_openclaw.md`

以上报告撰写时仍基于双机分工物理前提，内容本身不改，但读者须知其"MAC mini = 独立主机"的假设在 AMENDMENT-004 之后失效。
