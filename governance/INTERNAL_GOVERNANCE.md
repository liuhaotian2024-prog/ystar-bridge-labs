...
### Autonomous Mission — 自主长期任务 (Board directive, 2026-04-10)

#### 触发条件

Board 明确标记某个目标为 Autonomous Mission。标记方式：
- 指令里包含"Autonomous Mission"字样
- 或：指令里明确说明"按自主框架执行，不需要逐步指令"

**触发方式**：Board 通过 `gov_order.py` 发指令，指令文本包含
"Autonomous Mission"关键词。Agent 收到后自动识别，启动第十一条协议。
不需要任何额外的标记或审批流程。
...
| 阶段 | 等同级别 | 说明 |
|---|---|---|
| 认知建构阶段 | Level 1 | 自主执行，Board 批准框架报告后才进入执行 |
| 执行阶段内的单期决策 | Level 1 | 每周期内的战术决策自主做 |
| 跨周期策略调整 | Level 2 | 需 CEO 确认 |
| 资源调用 / 对外合作 / 预算 | Level 3 | 需 Board 批准 |
| 停止 Mission | Board only | Agent 不能自行停止 |

#### 持续时间

没有固定终点。Board 明确说停止，或 agent 主动上报无法继续，才结束。
每个执行周期结束后自动进入下一个周期。

#### Agent 主动上报停止的触发条件

- 连续 3 个周期迭代后仍未找到改进路径
- 遇到 `governance/ETHICS.md` 定义的禁忌无法绕过
- 资源/授权不足且 Board 未响应超过 72 小时

#### 前置条件（执行顺序保护）

`governance/WORKING_STYLE.md` 第十一条**必须已生效**。Agent 收到
Autonomous Mission 指令但第十一条不存在时：**拒绝执行**，通过
`reject_directive.py` 上报，原因："治理框架前置条件未满足"。

#### 伦理检查

Autonomous Mission 执行中的**每次对外发布前**必须对照
`governance/ETHICS.md` 逐条检查。不确定时默认不发布。

#### 来源

Board 2026-04-10 Autonomous Mission + 伦理边界 directive + 3 patches
（gov_order 触发方式 / 执行顺序保护 / Telegram 日报）。
