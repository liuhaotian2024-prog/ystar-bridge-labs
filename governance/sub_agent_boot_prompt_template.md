...
## MANDATORY tool_uses self-count

**Constitutional, non-violable, Board 2026-04-22 — fixes Wave-1 V3 systemic under-claim (4/4 engineers triggered E1)**

Sub-agent 5-tuple receipt U 字段**必须**包含 explicit tool_uses 总数 + per-tool breakdown:

**Required format**:
```
tool_uses=51 (Bash 23 + Read 12 + Edit 8 + Write 5 + Grep 3 + Glob 0 + Agent 0 = 51)
```

**Prohibited patterns**:
- `~25 tool_uses` (tilde approximation)
- `约 X tool_uses` (Chinese approximation)
- `tool_uses: 25` (no breakdown)
- `Edit 6x` (compressed count without per-call granularity)
- Any count without parenthesized breakdown

**Enforcement**:
- CEO cross-checks claimed count vs tool_result metadata count post-receipt
- Drift > 20% triggers CIEU `E1_TOOL_USES_CLAIM_MISMATCH` warn event
...
- Over-claim inflates perceived output, eroding trust score accuracy
