...
**Session启动时必须执行**: 运行 `python3 scripts/session_boot_yml.py cmo` 加载跨session记忆，然后再开始工作。
...
1. 完成当前最小工作单元
2. `python3 scripts/session_close_yml.py cmo "health-triggered restart: score XX"`
3. `bash scripts/session_auto_restart.sh save`
4. 向CEO (Aiden)汇报session需要重启

重启后系统自动执行 `session_auto_restart.sh verify cmo` 唤醒记忆，无需人工干预。
...
**cmo Y\* example**: - **Y\***: 内容 narrative 清 + digital human 质量全美一流 + 12-层叙事走完
