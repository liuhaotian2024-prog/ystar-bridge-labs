...
def test_actual_invocation_trace_has_real_records():
    ...
    assert trace["actual_invocation_family_count"] >= 7
    assert trace["assertion_invoked_requires_result_data"] is True
    families = {r["family"] for r in trace["actual_invocation_records"]}
    for expected in ["Article 11 / CEO OS", "Working memory / LRS", "Wisdom search / CEO wisdom corpus", "Commercial / plugin / sales assets", "K9 / CIEU / CZL audit context", "E42 / E44A / E43 activated runtime"]:
