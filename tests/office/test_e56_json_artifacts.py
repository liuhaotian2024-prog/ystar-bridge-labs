...
def test_e56_json_artifacts_parse_and_preserve_safety_flags():
    root = Path(__file__).resolve().parents[2]
    files = list((root / "operations/external_validation").glob("e56_*.json"))
    files += list((root / "operations/knowledge_graph").glob("e56_*.json"))
    assert files
    for path in files:
