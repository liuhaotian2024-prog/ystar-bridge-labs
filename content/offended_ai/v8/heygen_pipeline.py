#!/usr/bin/env python3
"""Sofia-CMO v8 HeyGen Avatar IV pipeline — fork of v5, expanded word count.

Changes vs v5:
- SCRIPT tightened/expanded to 170w targeting 57-60s at 3.0 wps (v5 was 142w
  = 50.82s, too short; v4 was 210w = 74.58s, too long).
- Avatar group name bumped to sofia_v8_latenight_20260413.
- Output filename episode_001_FINAL_60s_v8.mp4.
- Everything else (portrait, bg #1a1a1a, overlay lower-third, voice Allison,
  Avatar IV settings expressive+motion_strength=1.0) preserved verbatim
  from v5 — these are Board-approved.

Delete notes from v5 addendum applied:
- Skip 2b poll (405-returning endpoint, v5 confirmed 30 polls is a no-op).
- Avatar-group create then immediate generate (v5 addendum #5).

NEVER use Kling/Replicate lipsync — see v7/CMO_VERDICT_KLING_BLOCKER.md
for failure mode (male voice injection + German bg audio).
"""
import json, os, sys, time, subprocess, urllib.request, urllib.error, pathlib, shutil, hashlib

BASE = pathlib.Path("/Users/haotianliu/.openclaw/workspace/ystar-company/content/offended_ai/v8")
...
# --- 0b. Inherit + rebuild lower-third overlay ----------------------------
OVERLAY = BASE / "overlay_lower_third.png"
MKLT_SRC = V5 / "make_lower_third.py"
MKLT = BASE / "make_lower_third.py"
if not MKLT.exists():
    shutil.copy(MKLT_SRC, MKLT)
if not OVERLAY.exists():
    subprocess.run([sys.executable, str(MKLT)], check=True)
...
wc = len(SCRIPT.split())
print(f"[v8] script word count = {wc} (target 165-180 for 57-60s at 3.0 wps)")
...
def mk_group():
    return http("POST", "https://api.heygen.com/v2/photo_avatar/avatar_group/create",
                ...
                body={"name": "sofia_v8_latenight_20260413", "image_key": IMAGE_KEY})

grp = step("2. Create photo avatar group", mk_group)
TALKING_PHOTO_ID = grp["data"]["id"]
json.dump(grp, open("group_create.json", "w"))

# v5 addendum #5: 2b poll against 405-returning endpoint is a no-op — skip it.
# Allow small sleep so HeyGen internal dimension extraction finishes.
print("\n== 2b. Allow 60s for internal dimension extraction (per ep002 notes) ==")
time.sleep(60)
...
out = "episode_001_FINAL_60s_v8.mp4"
cmd = [
    "ffmpeg", "-y",
    "-i", "raw_heygen.mp4",
    "-i", str(OVERLAY),
...
# --- 7. Canonical copy ---------------------------------------------------
canonical = "/Users/haotianliu/.openclaw/workspace/ystar-company/content/offended_ai/episode_001_FINAL_60s_v8.mp4"
shutil.copy(out, canonical)
print(f"Copied -> {canonical}")

# --- 8. Duration probe + sha256 -----------------------------------------
try:
    r = subprocess.run(
